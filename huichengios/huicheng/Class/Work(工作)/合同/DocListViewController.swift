...
class DocListViewController: BaseViewController,UITableViewDataSource,UITableViewDelegate {
    let mainTabelView : UITableView = UITableView()
    var dataArr : [getdetailModel_document] = []
    ...
    override func viewDidLoad() {
        ...
        self.navigation_title_fontsize(name: "函件列表", fontsize: 18)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row < self.dataArr.count {
            cell.setData_doclist(model: dataArr[indexPath.row])
